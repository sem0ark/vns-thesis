...
def test_vns_initialization():
    ...
    optimizer = VNSOptimizer(
        ...
        shake_function=lambda sol, k: Solution(data_id=sol.data_id + k),
        acceptance_criterion=MagicMock(),
    )
    optimizer.problem.get_initial_solutions.return_value = [Solution(data_id=100)]
    optimizer.acceptance_criterion.get_one_current_solution.return_value = Solution(
        data_id=100
    )
    optimizer.reset()
