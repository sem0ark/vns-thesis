...
def example_problem_2d() -> MOKPProblem:
    return MOKPProblem(weights=[[1, 2, 3, 4, 5]], profits=[[5, 4, 3, 2, 1], [1, 2, 3, 4, 5]], capacity=3)


@pytest.fixture
def example_problem_2d2d() -> MOKPProblem:
    return MOKPProblem(weights=[[1, 2, 3, 4, 5], [1, 2, 3, 2, 1]], profits=[[5, 4, 3, 2, 1], [1, 2, 3, 4, 5]], capacity=[3, 3])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(np.array([1, 1, 1, 0, 0], dtype=int), False, id="Weight: 6 > 3 (Infeasible)"),
    ]
)
def test_is_feasible_1d(example_problem_1d: MOKPProblem, solution_data: np.ndarray, is_feasible: bool):
...
@pytest.mark.parametrize(
    ["solution_data", "is_feasible"],
    [
        pytest.param(np.array([1, 0, 0, 0, 0], dtype=int), True, id="Weights: [1, 1] <= [3, 3]"),
        pytest.param(np.array([0, 1, 0, 0, 0], dtype=int), True, id="Weights: [2, 2] <= [3, 3]"),
        pytest.param(np.array([1, 1, 1, 0, 0], dtype=int), False, id="Weights: [6, 6] > [3, 3] (Infeasible)"),
        pytest.param(np.array([0, 1, 1, 0, 0], dtype=int), False, id="Weights: [5, 5] > [3, 3] (Infeasible)"),
        pytest.param(np.array([0, 0, 0, 0, 1], dtype=int), False, id="Weights: [5, 1] > [3, 3] (Infeasible)"),
    ]
)
def test_is_feasible_2d2d(example_problem_2d2d: MOKPProblem, solution_data: np.ndarray, is_feasible: bool):
...
def test_evaluate_1d(example_problem_1d: MOKPProblem, solution_data: np.ndarray, objectives: tuple[float, ...]):
    assert _MOKPSolution(solution_data, example_problem_1d).objectives == pytest.approx(objectives)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(np.array([1, 1, 1, 0, 0], dtype=int), (0.0, 0.0), id="Infeasible 1"),
    ]
)
def test_evaluate_2d(example_problem_2d: MOKPProblem, solution_data: np.ndarray, objectives: tuple[float, ...]):
    assert _MOKPSolution(solution_data, example_problem_2d).objectives == pytest.approx(objectives)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(np.array([1, 1, 1, 0, 0], dtype=int), (0.0, 0.0), id="Infeasible 1"),
        pytest.param(np.array([0, 1, 1, 0, 0], dtype=int), (0.0, 0.0), id="Infeasible 2"),
    ]
)
def test_evaluate_2d2d(example_problem_2d2d: MOKPProblem, solution_data: np.ndarray, objectives: tuple[float, ...]):
    assert _MOKPSolution(solution_data, example_problem_2d2d).objectives == pytest.approx(objectives)
