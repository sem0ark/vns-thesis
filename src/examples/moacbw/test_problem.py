...
def test_example_problem(example_problem: MOACBWProblem):
    antibandwidth = 2
    cutwidth = 5
    antibandwidth_obj, cutwidth_obj = _MOACBWSolution(np.array([0, 3, 2, 6, 5, 1, 4]), example_problem).objectives
...
def test_solution_distance_partial(example_problem: MOACBWProblem):
    ...
    assert example_problem.calculate_solution_distance(sol1, sol2) == pytest.approx(2 / 7)
