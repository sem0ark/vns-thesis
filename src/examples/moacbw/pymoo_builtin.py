...
from src.examples.moacbw.problem import MOACBWProblem, MOACBWProblemPymoo


def run_pymoo(
    instance_path: str, run_seconds: float, name: str, algorithm: NSGA2 | SPEA2
):
    problem_instance = MOACBWProblem.load(instance_path)
    problem = MOACBWProblemPymoo(problem_instance)
...
def register_cli(cli: Any) -> None:
    cli.register_runner("moacbw", list(prepare_optimizers()))
